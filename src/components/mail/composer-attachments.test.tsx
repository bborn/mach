...
/**
 * A forward's files are named before they are here.
 *
 * The bytes are a download and an upload away, and the composer opens without
 * waiting for them — so between the draft appearing and the rows landing there
 * is a window in which a composer with nothing in its list would be
 * indistinguishable from the bug this fixes, which dropped the files silently.
 */
describe("the files a forward is still fetching", () => {
  const invoice = { filename: "invoice.pdf", mimeType: "application/pdf", sizeBytes: 158204 };

  it("are named and sized while they are on their way", () => {
    const html = markup({}, { arriving: [invoice] });
    expect(html).toContain("invoice.pdf");
    expect(html).toContain("154 KB");
    expect(html).toContain("fetching");
  });

  it("stop being drawn once the row they were promising lands", () => {
    const host = parse(
      markup(
        { attachments: [file({ id: "a", filename: "invoice.pdf" })] },
        { arriving: [invoice] },
      ),
    );
    const names = [...host.querySelectorAll("li span[title]")].map((s) => s.textContent);
  ...
  it("offer nothing to press, because there is nothing there yet to remove", () => {
    const host = parse(markup({}, { arriving: [invoice] }));
    const chip = host.querySelector("li");
    expect(chip?.textContent).toContain("invoice.pdf");
    expect(chip?.querySelectorAll("button")).toHaveLength(0);
  });

  it("draw the list at all, on a draft that has no rows yet", () => {
    expect(markup({}, { arriving: [] })).not.toContain("fetching");
    expect(markup({}, { arriving: [invoice] })).toContain("<ul");
  });
});
