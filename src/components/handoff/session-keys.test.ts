...
function app(keymap: Keymap) {
  ...
  // The account rail's pair, which the tab strip does the same thing to.
  keymap.register({
    keys: "mod+alt+right",
    allowInInput: true,
    description: "Sidebar wider",
    handler: fired.railWider,
  });
  keymap.register({
    keys: "mod+alt+left",
    allowInInput: true,
    description: "Sidebar narrower",
    handler: fired.railNarrower,
  });
  return fired;
}
...
describe("the session pane's keyboard contract", () => {
  ...
  beforeEach(() => {
    ...
    fired = app(keymap);
    pane = { live: true, focused: true, finished: false, tabs: 1, paletteOpen: false };
    actions = {
      toggleFocus: vi.fn<() => void>(),
      end: vi.fn<() => void>(),
      nextTab: vi.fn<() => void>(),
      previousTab: vi.fn<() => void>(),
      taller: vi.fn<() => void>(),
      shorter: vi.fn<() => void>(),
    };
  ...
  it("steps through the tabs on \u2325\u2318\u2190 and \u2325\u2318\u2192, rather than resizing the sidebar", () => {
    pane.tabs = 3;
    keymap.handle(press("ArrowRight", { meta: true, alt: true }));
    keymap.handle(press("ArrowLeft", { meta: true, alt: true }));

    expect(actions.nextTab).toHaveBeenCalledTimes(1);
    expect(actions.previousTab).toHaveBeenCalledTimes(1);
    expect(fired.railWider).not.toHaveBeenCalled();
    expect(fired.railNarrower).not.toHaveBeenCalled();
  });

  it("gives the pair back to the sidebar when there is only one tab", () => {
    // One session is not a strip, and the key it would answer to is somebody
    // else's the rest of the time.
    pane.tabs = 1;
    keymap.handle(press("ArrowRight", { meta: true, alt: true }));
    expect(actions.nextTab).not.toHaveBeenCalled();
    expect(fired.railWider).toHaveBeenCalledTimes(1);
  });

  it("gives the pair back to the sidebar once the terminal is not focused", () => {
    pane.tabs = 3;
    pane.focused = false;
    keymap.handle(press("ArrowLeft", { meta: true, alt: true, tagName: "DIV" }));
    expect(actions.previousTab).not.toHaveBeenCalled();
    expect(fired.railNarrower).toHaveBeenCalledTimes(1);
  });

  it("closes the tab in front on \u21e7\u2318W, whether or not its process is still running", () => {
    pane.tabs = 3;
    keymap.handle(press("w", { meta: true, shift: true }));
    expect(actions.end).toHaveBeenCalledTimes(1);
  ...
  it("still reaches the strip with \u21e7\u2318T after the front tab's process has exited", () => {
    // Otherwise a finished tab would be a dead end: the emulator has dropped
    // focus, and the step keys need it.
    pane.live = false;
    pane.focused = false;
    pane.finished = true;
    keymap.handle(press("t", { meta: true, shift: true, tagName: "DIV" }));
