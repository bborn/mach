 ...
 * | ⇧⌘W | close the tab in front, from anywhere |
 * | ⌥⌘← ⌥⌘→ | the previous tab, the next tab — while the terminal has focus |
 ...
 *
 * # Why the tabs are ⌥⌘← and ⌥⌘→
 *
 * ⌃⇥ and ⇧⌘[ are what a browser uses, and neither is available: ⌃ belongs to
 * the process by the rule above, and this app has no ⌘-bracket vocabulary to
 * join. ⌥⌘←/→ is already the app's "step sideways through a list of things" —
 * the account rail steps accounts with it — so a second surface claiming it
 * while *it* has focus is the arrangement ⌥⌘↑/↓ already has with the agent
 * drawer and the composer. The priority says who wins, which is why
 * `keymap.conflicts()` reports no tie.
 *
 * They are live only while the terminal has focus, so the rail keeps them
 * everywhere else. ⇧⌘T is therefore gated on the pane being *present* rather
 * than running: a tab whose process has exited must still be reachable, or the
 * only way back into the strip would be the mouse.
 *
...
export interface SessionKeyState {
  /** The tab in front has a process that is still running. */
  live: () => boolean;
  /** The emulator holds focus. */
  focused: () => boolean;
  /** There is a pane at all — running or finished. */
  present: () => boolean;
  /** The process in front has exited and its tab is a receipt. */
  finished: () => boolean;
  /** How many tabs. One is not a strip, and does not answer the step keys. */
  tabs: () => number;
  paletteOpen: () => boolean;
}
...
export function sessionBindings(
  ...
  return [
    ...
    },
    {
      ...
      // `present`, not `live`: a finished tab is still a tab, and the strip has
      // to be reachable from the keyboard whatever state the front one is in.
      when: () => state.present(),
    ...
    },
    {
      keys: "shift+mod+w",
      group: "Session",
      description: "Close the session tab",
      allowInInput: true,
      priority: SESSION_CHORD_PRIORITY,
      when: () => state.present(),
      handler: () => actions.end(),
    },
    {
      ...
      allowInInput: true,
      // Above the account rail's identical pair, and gated on the terminal
      // having focus so the rail keeps them everywhere else. Same arrangement
      // as ⌥⌘↑/↓ below; see the file's doc.
      priority: SESSION_CHORD_PRIORITY,
      when: () => state.focused() && state.tabs() > 1,
