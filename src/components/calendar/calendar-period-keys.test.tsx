...
describe("⇧← and ⇧→ on the calendar", () => {
  ...
  it("prints both meanings on the shortcut sheet", async () => {
    // One key, two truthful rows — the point of the test. The Event row now
    // pairs the two arrows into a single row through `alsoKeys` rather than
    // printing "Move to the next day" and "…previous day" separately, so ⇧→
    // reaches the sheet as the second chip of the ⇧← row.
    await mount([]);
    const rows = probe()
      .keymap.active()
      .filter((b) => [b.keys, ...(b.alsoKeys ?? [])].includes("shift+right") && b.description)
      .map((b) => `${b.group}: ${b.description}`);
    expect(rows.sort()).toEqual(["Calendar: Next period", "Event: Move by a day"]);
  });
