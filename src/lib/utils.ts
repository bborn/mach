 ...
 * `text-reading`, `text-title` — so out of the box the merger filed them under
 * *text colour*, the only group whose prefix matches. That made `cn("text-micro
 ...
 *
 * **A step added to the ramp has to be added here too.** `text-title` is the
 * newest. A size the merger has not been told about fails the same silent way
 * the whole scale did.
 */
const twMerge = extendTailwindMerge({
  extend: {
    classGroups: {
      "font-size": [{ text: ["micro", "list", "body", "reading", "title"] }],
    },
  },
});
