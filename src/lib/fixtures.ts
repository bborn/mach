...
/**
 * Two surfaces that a browser tab cannot otherwise reach, behind a flag.
 *
 * The agent drawer and the handoff sheet are the only parts of the app with no
 * fixture behind them: both go straight to Rust, so `bun run dev` renders an
 * empty dock and a dialog that fails on open. That also makes them the two
 * surfaces nobody can photograph — `scripts/qa` can press keys and click, but
 * it cannot type, and both are reached by typing a sentence into ⌘K.
 *
 * They are behind `?showcase=` rather than always on because the drawer is 320
 * pixels of window: a session appearing in every `bun run dev` would be in the
 * way of the component work the fixture app exists for.
 *
 *   bun run dev  →  http://localhost:1420/?showcase=agent
 *                   http://localhost:1420/?showcase=handoff
 *
 * The transcript below is not invented freely. Every tool name, every summary
 * string and the approval sentence are the formats `src-tauri/src/agent/`
 * actually emits — `tools.rs` for the outcomes, `gate.rs` for the two
 * summaries — so what is on screen is the shape of a real run over this
 * mailbox, not a mock-up of one.
 */
...
const AGENT_THREAD = threads[0]!;

export const AGENT_DRAFT_ID = "draft-showcase-1";

export const agentSession = {
  id: "showcase-1",
  title: "What is Tawny waiting on, and can you answer it?",
  ...
  context: [
    {
      ...
      label: AGENT_THREAD.subject,
      threadId: AGENT_THREAD.id,
      detail: "Tawny Reeves",
    },
    { id: "mailbox", kind: "mailbox" as const, label: "Inbox · All accounts" },
  ],
  entries: [
    { role: "user" as const, text: "What is Tawny waiting on, and can you answer it?" },
    {
      role: "tool" as const,
      id: "t1",
      name: "search_threads",
      summary: "4 matches for “data room”",
      state: "ok" as const,
    },
    {
      role: "tool" as const,
      id: "t2",
      name: "get_thread",
      summary: `Read “${AGENT_THREAD.subject}”`,
      state: "ok" as const,
    },
    {
      role: "tool" as const,
    ...
    },
    {
      role: "agent" as const,
      text:
        "She wants the cohort tab split into paid and organic before Thursday. " +
        "The data room walkthrough is Thursday at 2pm, so the deadline is that meeting. " +
        "I have written a reply saying the split lands Wednesday.",
    },
    {
      role: "tool" as const,
      id: "t4",
      name: "draft_reply",
      summary: `Drafted “${AGENT_THREAD.subject}”`,
      state: "ok" as const,
      artifact: {
        kind: "draft" as const,
        draftId: AGENT_DRAFT_ID,
        threadId: AGENT_THREAD.id,
        accountId: AGENT_THREAD.accountId,
        label: AGENT_THREAD.subject,
      },
    },
  ],
  pending: {
    toolUseId: "t5",
    name: "send_draft",
    summary: `Send “${AGENT_THREAD.subject}” to tawny@northloop.example now`,
    input: { draftId: AGENT_DRAFT_ID },
  },
};
...
export const handoffTargets = [
  ...
  },
  {
    ...
    name: "Notes",
    dir: "~/Notes",
    run: "note new --title {{subject}}",
    mode: "inline" as const,
    lastRunAt: Date.now() - 3 * DAY,
  },
];

export const handoffPreview = {
  targetId: "offerlab",
  targetName: "OfferLab",
  mode: "terminal" as const,
  dir: "/Users/alex/Projects/offerlab",
  command: "claude 'Pick this up: split the cohort tab by paid and organic'",
  argv: ["claude", "Pick this up: split the cohort tab by paid and organic"],
  contextLabel: AGENT_THREAD.subject,
  contextFile: "/tmp/mach-handoff/re-series-a-data-room.md",
  unproven: true,
  prompt: [
    "Pick this up: split the cohort tab by paid and organic",
    "",
    "# Re: Series A data room — a few gaps",
    "From: Tawny Reeves <tawny@northloop.example>",
    "To: alex@northwind.example",
    "",
    "The cohort tab still shows blended retention. Can you split paid vs organic",
    "before Thursday? Otherwise this looks right.",
    "",
    "Following up on this so it doesn't get lost. I've pulled the numbers into the",
    "shared sheet and flagged the three rows that don't reconcile — they're all in",
    "the same date range, which makes me think it's an export boundary problem",
    "rather than anything real.",
    "",
    "The short version: we can do it, but not before the end of the month, and",
    "only if the other thing slips. I'd rather make that trade explicitly than",
    "discover it in two weeks.",
    "",
    "Attachment: cohort-retention-q3.xlsx (180 KB)",
  ].join("\n"),
};
