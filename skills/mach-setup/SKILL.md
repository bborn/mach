---
name: mach-setup
description: Set up Mach's Google OAuth credentials end to end — create a Google Cloud project, enable the Gmail and Calendar APIs, configure the Google Auth Platform consent screen, create a Desktop app OAuth client, add the six scopes, publish to production so refresh tokens stop expiring after 7 days, write .env.local, and verify with the oauth_smoke example. Use when someone is setting up or re-setting-up Mach, hits "not configured", sees "Google hasn't verified this app", is being asked to re-authorize every week, or asks how to get Mach's Google credentials.
---
...
## Step 5 — Add the six scopes

`https://console.cloud.google.com/auth/scopes?project=<id>` → **Add or remove
scopes**. Paste all six into the "manually add scopes" box, comma- or
...
2. **All six scopes** are listed. A missing one means Step 5 did not save.
