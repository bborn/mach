 ...
 *
 * This runs at plugin-host boot in the real app, not only in CI: the WebView is
 * a moving target, and a policy that held in one OS release is not a policy that
 * holds in the next. If any row comes back `allowed: true`, the host refuses to
 * load plugins at all and says which check failed.
 *
