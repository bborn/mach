 ...
 *
 * The worker is a *scheduling* boundary, not the security one: it keeps plugin
 * CPU off the UI thread and denies the plugin a `document`. The origin and the
 * CSP are what buy the isolation. See `docs/plugins.md` §2.
 */
 ...
 * disagreeing.
 *
 * `now()` is local and synchronous because every other route makes formatting a
 * date an async operation. The host hands over an offset at load time, so a
 * test can freeze the clock without the plugin knowing.
 */
let clockOffset = 0;

function makeMach() {
  const mach = { now: () => Date.now() + clockOffset };
  ...
  for (const key of Object.keys(mach)) {
    if (typeof mach[key] === "object") Object.freeze(mach[key]);
  }
...
self.addEventListener("message", async (event) => {
  ...
  if (message?.t === "load") {
    try {
      clockOffset = Number(message.clockOffset ?? 0) || 0;
      ...
      self.postMessage({
        ...
        value: {
          ...
          decorations: Object.keys(plugin.decorations ?? {}),
        },
      });
    } catch (error) {
      self.postMessage({
